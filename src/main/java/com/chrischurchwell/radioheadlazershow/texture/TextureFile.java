 ...
 */
package com.chrischurchwell.radioheadlazershow.texture;
...
public enum TextureFile {
	...
	public Texture getTexture() {
		...
		return new Texture(RadioheadLazerShow.getInstance(), this.file, this.width, this.height, this.size);
		
	}
	
	public static void extractTextures() {
		File dir = new File(RadioheadLazerShow.getInstance().getDataFolder(), "textures");
		...
		for (TextureFile texture : TextureFile.values()) {
			...
			RadioheadLazerShow.getInstance().saveResource(fileCopyRelPath, true);
		}
	}
	...
	public static void preCacheTextures() {
		...
		SpoutManager.getFileManager().removeFromCache(RadioheadLazerShow.getInstance(), textures);
		
		//add
		for (TextureFile texture : TextureFile.values()) {
			SpoutManager.getFileManager().addToPreLoginCache(
					RadioheadLazerShow.getInstance(),
					new File(RadioheadLazerShow.getInstance().getDataFolder(), new File("textures", texture.getFile()).getPath())
					);
		}
	}
